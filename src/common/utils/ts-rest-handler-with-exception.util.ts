...
import { AppRoute, ServerInferRequest, ServerInferResponses } from '@ts-rest/core';
import { TsRestException } from '@ts-rest/nest';
import type { Request } from 'express-serve-static-core';

type TsRestRequestShape<TRoute extends AppRoute> = ServerInferRequest<TRoute, Request['headers']>;
 ...
 * ```
 */
export function tsRestHandlerWithException<T extends AppRoute>(route: T, handler: (args: TsRestRequestShape<T>) => Promise<ServerInferResponses<T>>): (args: TsRestRequestShape<T>) => Promise<ServerInferResponses<T>> {
  return async (args: TsRestRequestShape<T>): Promise<ServerInferResponses<T>> => {
    try {
      ...
      const message = e instanceof HttpException ? e.message : (e as Error).toString();
