...
import {
  ...
  TokenSignResult,
} from '@/common/token/application/core/abstract-token.service';
...
export class TokenService implements AbstractTokenService {
  ...
  async getAccessToken(instance?: string): Promise<string> {
    ...
    const prefix = isKonga
      ? TOKEN_MODULE_CONFIG.CACHE_KEY_PREFIX.KONGA
      : TOKEN_MODULE_CONFIG.CACHE_KEY_PREFIX.KPAY;
    const cacheKey = `${prefix}-${appName}`;

    const cached = await this.tokenCache.get(cacheKey);
    if (cached) return cached;

    const oauthUrl = isKonga
      ? this.configService.get<string>('oauthKongaUrl')
      : this.configService.get<string>('oauthUrl');
    const clientId = isKonga
      ? this.configService.get<string>('oauthKongaClientId')
      : this.configService.get<string>('oauthClientId');
    const clientSecret = isKonga
      ? this.configService.get<string>('oauthKongaClientSecret')
      : this.configService.get<string>('oauthClientSecret');
  ...
  async signToken(payload: Record<string, unknown>): Promise<TokenSignResult> {
    const expiresIn =
      Number(this.configService.get<string>('tokenTTL')) ||
      TOKEN_MODULE_CONFIG.DEFAULT_JWT_EXPIRY_SECONDS;
